...
//=============================================================================
/*:
 * @plugindesc 3 - Generates dungeon levels during runtime.
 * @author Alex
 *
...
/**
 * @class
 * @description A non-randomly generated piece of dungeon
 */
PD.Generator.CustomFeature=function(options){
    PD.Generator.CustomFeature.prototype.initialize.call(this, options);
};
/**
 * @function
 * @private
 */
PD.Generator.CustomFeature.prototype.initialize=function(options){
    ...
    for(var propertyName in PD.Generator.CustomFeature.Defaults) {
        this["_"+propertyName] = options[propertyName]?options[propertyName]:PD.Generator.CustomFeature.Defaults[propertyName];
    }
}
PD.Generator.CustomFeature.prototype.height=function(){
    return this._data.length;
}
PD.Generator.CustomFeature.prototype.width=function(){
    return this._data.length>0?this._data[0].length:0;
}
/**
 * @struct
 * @description Default values for a CustomFeature
 */
PD.Generator.CustomFeature.Defaults={
    //Dungeon map width
...
/**
 * @class
 * @description A dungeon that can be regenerated during runtime.
 */
PD.Generator.Dungeon=function(options){
    PD.Generator.Dungeon.prototype.initialize.call(this, options);
};
/**
 * @struct
 * @description Default values for a Dungeon
 */
PD.Generator.Dungeon.Defaults={
    //Dungeon map width
    width:17,
    //Dungeon map height
    height:13,
...
PD.Generator.Dungeon.prototype.initialize=function(options){
    if(options==undefined || options==null){
        options={};
    }
    for(var propertyName in PD.Generator.Dungeon.Defaults) {
        this["_"+propertyName] = options[propertyName]?options[propertyName]:PD.Generator.Dungeon.Defaults[propertyName];
    }
    this.preGenerate(0);
    this.resetValues();
}

PD.Generator.Dungeon.prototype.resetValues=function(){
    this._entrance=null;
    this._exit=null;
    this._rooms=[];
...
PD.Generator.Dungeon.prototype.getRandomCustomFeature=function(filter){
    ...
    return PD.Helpers.randomFromWithWeight(filtered, "_weight");
}
PD.Generator.Dungeon.prototype.getCustomFeatures=function(filter){
    if(filter==undefined || filter==null){
        filter=function(cf){
            return true;
        }
...
PD.Generator.Dungeon.prototype.getRandomRoom=function(filter){
    if(filter==undefined || filter==null){
        filter=function(cf){
            return true;
        }
    ...
    return PD.Helpers.randomFrom(filtered);
}
//Returns a random corridor from the dungeon
PD.Generator.Dungeon.prototype.getRandomCorridor=function(filter){
    if(filter==undefined || filter==null){
        filter=function(cf){
            return true;
        }
...
PD.Generator.Dungeon.prototype.getRandomGeneratedRoom=function(filter){
    if(filter==undefined || filter==null){
        filter=function(cf){
            return true;
        }
...
PD.Generator.Dungeon.prototype.getRandomCellInRoom=function(room, cellFilter){
    ...
    return susCells.length>0?PD.Helpers.randomFrom(susCells):null;
}
PD.Generator.Dungeon.prototype.getCellsInRoom=function(room){
    var lst=[];
...
PD.Generator.Dungeon.prototype.fillPendingFeatures=function(){
    var cFeatures=[];
    ...
    this._pendingFeatures=PD.Helpers.shuffleArray(cFeatures);
}
//Checks if the generated dungeon is valid for the configuration given.
PD.Generator.Dungeon.prototype.validate=function(){
...
PD.Generator.Dungeon.prototype.generate=function(){
    var isValid=false;
    var exitLoop=false;
...
PD.Generator.Dungeon.prototype.generateGOs=function(){
    var slf=this;
...
PD.Generator.Dungeon.prototype._makeFirstRoom=function(){
    for (var i = 0; i < 1000; i++) {
        if(this._makeRoom(Math.floor(this._width/2), Math.floor(this._height/2),PD.Helpers.randomFrom([8,6,4,2]), true, this._roomWallChance, null, null)){
            return true;
        }
    }
    return false;
}
PD.Generator.Dungeon.prototype.print=function(asSymbols){
    var res="";
    for (var y = 0; y < this._height; y++) {
        for (var x = 0; x < this._width; x++) {
            if(asSymbols){
...
PD.Generator.Dungeon.prototype._getWallHeight=function(side){
    return (this._wallHeight[side]==-1?0:this._wallHeight[side]);
}
PD.Generator.Dungeon.prototype._leaveGapBetweenFeatures=function(side){
    return !(this._wallHeight[side]==-1);
}
PD.Generator.Dungeon.prototype._makeRoom=function(x, y, direction, isFirstRoom, wallChance, connectedRoom, connectedCorridor){
    ...
    if(this._pendingFeatures.length>0){
        var makeCustomPendingFeature=PD.Helpers.randomChance(this._customFeatureChance);
    ...
    var useWall=PD.Helpers.randomChance(wallChance);
    if(customFeatureToCreate!=null){
        roomRect.width=customFeatureToCreate.width;
        roomRect.height=customFeatureToCreate.height;
    }else{
        roomRect.width=PD.Helpers.randomInteger(this._minRoomSize,this._maxRoomSize);
        roomRect.height=PD.Helpers.randomInteger(this._minRoomSize,this._maxRoomSize);
    }
    ...
    if(this._tryPlaceRect(roomRect,useWall)){
        ...
        if(customFeatureToCreate!=null){
            ...
            //newData=PD.Helpers.transposeArray(newData);
        }
...
PD.Generator.Dungeon.prototype._storeRectExits=function(rect, dir, isFirstRoom){
...
PD.Generator.Dungeon.prototype._tryPlaceRect=function(rect, doCheckWallsAlso){
...
PD.Generator.Dungeon.prototype._placeRect=function(rect, dataOrTileId, doAddWalls, connectedRoom, connectedCorridor){
    if (rect.x < this._getWallHeight(4) || rect.y < this._getWallHeight(8) || rect.x + rect.width > this._width - this._getWallHeight(6) || rect.y + rect.height > this._height - this._getWallHeight(2)){
        return false;
    }
    var newData=[];
...
PD.Generator.Dungeon.prototype._createCorridorConsistentFeature=function(){
    ...
    var side=PD.Helpers.randomFrom(previousCorridor.exits);
    var x=PD.Helpers.randomInteger(side.x, side.x + side.width-1)
    var y=PD.Helpers.randomInteger(side.y, side.y + side.height-1);
...
PD.Generator.Dungeon.prototype._createFeature=function(){
    //for (var i = 0; i < 1000; i++) {
        ...
        var side=null;
        if(PD.Helpers.randomChance(50) || this._corridors.length==0){
            previousRoom=this.getRandomRoom();
            side=PD.Helpers.randomFrom(previousRoom.exits);
        }else{
            previousCorridor=this.getRandomCorridor();
            side=PD.Helpers.randomFrom(previousCorridor.exits);
        }

        //var sidePositionIndex=PD.Helpers.randomInteger(this._exits.length-1);
        //var x=PD.Helpers.randomInteger(this._exits[sidePositionIndex].x, this._exits[sidePositionIndex].x + this._exits[sidePositionIndex].width-1);
        //var y=PD.Helpers.randomInteger(this._exits[sidePositionIndex].y, this._exits[sidePositionIndex].y + this._exits[sidePositionIndex].height-1);
        var x=PD.Helpers.randomInteger(side.x, side.x + side.width-1)
        var y=PD.Helpers.randomInteger(side.y, side.y + side.height-1);
...
PD.Generator.Dungeon.prototype._tryCreateFeature=function(x, y, dir, connectedRoom, connectedCorridor){
    var dx=0;
    var dy=0;
    ...
    var shouldAddRoom=(PD.Helpers.randomChance(this._roomChance+extraChances));
...
PD.Generator.Dungeon.prototype._makeCorridor=function(x, y, dir, wallChance, connectedRoom, connectedCorridor){
    var corridorRect={
        x:0,
        y:0,
        width:0,
    ...
    if(!forceHorizontal && !forceVertical){
        forceHorizontal=PD.Helpers.randomChance(50);
        forceVertical=!forceHorizontal;
    }
    if(forceHorizontal){
        //Horizontal corridor
        corridorRect.width=PD.Helpers.randomInteger(this._minCorridorLength,this._maxCorridorLength);
        corridorRect.height=1;
        if(dir==8){           //North
            corridorRect.y = y - 1;
            //ToDo: xq coño se hace esto?...
            if(PD.Helpers.randomInteger(100)<=50){
        ...
        corridorRect.height=PD.Helpers.randomInteger(this._minCorridorLength,this._maxCorridorLength);
        if(dir==8){           //North
...
if(PluginManager!=undefined){
    ...
    PD.Generator.Helpers={};
    PD.Generator.Helpers.rotateMapToRight=function(array) {
        var result = [];
    ...
    PD.Generator.Helpers.rotateMapToLeft=function(array) {
        var result = [];
        array.forEach(function (a, i, aa) {
            a.forEach(function (b, j, bb) {
                result[j] = result[j] || [];
